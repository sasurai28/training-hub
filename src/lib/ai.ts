...
- 「ユーザーの現在の方針」が与えられたら、それを最優先で尊重してアドバイスする。`
...
export function buildCoachContext(data: Pick<AppData, 'logs' | 'settings'>, today: string): string {
  ...
  // ユーザーの現在の方針（最優先で尊重させる）
  if (settings.policy?.trim()) {
    lines.push('## ユーザーの現在の方針（最優先で尊重すること）')
    lines.push(settings.policy.trim())
  }
...
- **「ユーザーの現在の方針」が与えられたら、それを最優先で尊重して計画を組む**（目標やセオリーと衝突する場合は方針を優先しつつ summary で補足）。
